            ...
            loop {
                ...
                if self.0[1] == descriptor_type {
                    if self.0[0] >= min_len {
                        ...
                        warn!(
                            "ignoring descriptor of type {} and length {} because the minimum length is {}",
    ...
    (impl $(<$( $i_lt:lifetime ),+>)? $tname:ident $(<$( $t_lt:lifetime ),+>)? {
        ...
        impl $(<$( $i_lt ),+>)? $tname $(<$( $t_lt ),+>)? {
            $(
                $(#[$attr])*
                #[inline]
                $vis fn $name(&self) -> $ty { <$ty>::from_le_bytes(self.0[$pos..$pos + core::mem::size_of::<$ty>()].try_into().unwrap()) }
            )*
        }
    }
}
...
pub(crate) fn parse_concatenated_config_descriptors(
    mut buf: &[u8],
) -> impl Iterator<Item = ConfigurationDescriptor<'_>> + '_ {
    iter::from_fn(move || {
